...
population in Kruger National Park, South Africa. The carrying capacity of the park is 
...
# Run the program from the Terminal by entering "python3 elephant.py"
...
def newElephant (parameters, age):          #takes parameters and age as arguments
...
def calcSurvival(parameters, population):                   #takes parameters and population as arguments
    new_population = []                                     #create an empty list new_population
    #print(population)                                   
    for elephant in population:                             #loop over the existing population list
        #calves
        if (elephant[IDXAge] == 1):     #if the elephant is a calf
            if random.random() < parameters[IDXcalfprob]:   #survival probability
                new_population.append(elephant)             #add the elephant to the new_population list
        #juveniles and adults
        elif (elephant[IDXAge]<=parameters[IDXmaxage]):     #if the elephant is a juvenile or an adult
            if random.random() < parameters[IDXadultprob]:  #survival probability
                new_population.append(elephant)             #add the elephant to the new_population list
        #seniors
        else:   #if the elephant is a senior
            #print("senior")
            if random.random() < parameters[IDXseniorprob]: #survival probability
                new_population.append(elephant)             #add the elephant to the new_population list
...
age (maxage)."""    
def dartElephants(parameters, population):                              #takes parameters and population as arguments
    for elephant in population:                                         #loop over the existing population
        if elephant[IDXGender] == "f":                                  #if the elephant is a female
            if elephant[IDXAge] > parameters[IDXjuvage]:               #if the elephant is older than juvenile age
                if elephant[IDXAge] < parameters[IDXmaxage]:           #if the elephant is younger than senior age
                    if random.random() < parameters[IDXdarted]:         #test the darting probability
                        elephant[IDXMonthsPregnant] = 0                 #set the pregnancy field to 0
                        elephant[IDXMonthsContraceptiveRemaining] = 22  #set the months of contraceptive left field to 22
    #print("dart")
    return population                                                   #return the population list after the loop
...
def cullElephants (parameters, population):         #takes parameters and population as arguments
    capacity = parameters[IDXcapacity]              #create a local variable for capacity
    culled = (len(population)-capacity)                       #number of animals that need to be culled
    if len(population) > capacity:                            #if there are too many elephants
        newpopulation = random.shuffle(population)  #randomly shuffle the population
        newpopulation = population[0:capacity]
    else:
        newpopulation = population
    return (newpopulation, culled)                  #return a tuple containing this new list and then the number of animals culled
...
def controlPopulation(parameters, population):
    ...
    if parameters[IDXdarted] == 0:
        #print("darted" , parameters[IDXdarted])
    ...
    for elephant in population:
        ...
        if gender == "f" and ((age > parameters[IDXjuvage]) and (age <= parameters[IDXmaxage])):
            ...
            # else
                ...
                if random.random() < (1.0 / (parameters[IDXcalv]*12 - 22)):
...
def simulateYear(parameters, population):       #takes parameters and population as arguments
    new_population = calcSurvival(parameters, population)        #call calcSurvival
    incrementAge(parameters, new_population)        #call incrementAge
    for i in range(12):                         #loop twelve times
        simulateMonth(parameters, new_population)   #call simulateMonth
    return(new_population)
...
def calcResults(parameters, population, culled):        #takes parameters, population, and culled as arguments
    calves = 0                                          #initialize the global variables
    juveniles = 0
    adultmales = 0
    adultfemales = 0
    seniors = 0
    pop=[]                                             #create an empty list test
    for elephant in population: 
        #print(elephant[IDXAge])                        #loop through the population
        if (elephant[IDXAge] == 1): #if the elephant is a calf
            calves = calves + 1                         #increment the number of calves by 1
        if (elephant[IDXAge]<=parameters[IDXjuvage]):   #if the elephant is a juvenile
            juveniles = juveniles + 1                   #increment the number of juveniles by 1
        if (elephant[IDXAge]>parameters[IDXjuvage]) and (elephant[IDXAge]<=parameters[IDXmaxage]):   #if the elephant is an adult
            if elephant[IDXGender] == "m":              #if the elephant is male
                adultmales = adultmales+1               #increment the number of adult males by 1
            if elephant[IDXGender] == "f":              #if the elephant is female
                adultfemales = adultfemales+1           #increment the number of adult females by 1
        if (elephant[IDXAge] > parameters[IDXmaxage]): #if the elephant is a senior
            seniors = seniors + 1                       #increment the number of seniors by 1
    
    pop = [len(population), calves, juveniles, adultmales, adultfemales, seniors, culled]
    
    """total = calves+juveniles+adultfemales+adultmales+seniors    #add them together to create the total
    #append each variable to the empty list
    test.append(total)  
    test.append(calves)
    ...
    test.append(culled)
    #print(test)"""
    return(pop)    #return the pop list
...
def runSimulation(parameters):
    ...
    #print("runsimulation", population)
...
def defaultParameters():
    ...
    theparameters = [calv, darted, juvage, maxage, calfprob, adultprob, seniorprob, capacity, years]
    return theparameters

def elephantSim(darted, inputParameters=None):
    results = []
    total=0
    years=0
    ...
    parameters[IDXdarted] = darted
    for i in range(4):
        results = results + runSimulation(parameters) 
    for i in range(len(results)):
        total = total + (results[i][0])
        #print(results[i][0])
    avgtotal = (total/len(results))
    return int(parameters[7] - avgtotal)
...
def main(argv):
    if (len(sys.argv)<1):                           #usage statement
        print("enter the probability of darting")
    darted = float(sys.argv[1])
    # assign each parameter from the table above to a variable with an informative name
    calv = 3.1
    juvage = 12
    maxage = 60
    ...
    adultprob = 0.996
    seniorprob = 0.200
    capacity = 1000			#CHANGE THIS BACK TO 1000
    years = 200
    total = 0
    # make the parameter list out of the variables
    parameters2 = []
    parameters2 = [calv, darted, juvage, maxage, calfprob, adultprob, seniorprob, capacity, years]
    results = []    
    results = runSimulation(parameters2)
    #print(results[-1])
    ...
    for i in range(len(results)):
        #print("Total population value in year ", i, ": ", results[i][0])
        calves += results[i][1]
        juveniles += results[i][2]
        adultmales += results[i][3]
        adultfemales += results[i][4]
        seniors += results[i][5]
        total += results[i][0]
